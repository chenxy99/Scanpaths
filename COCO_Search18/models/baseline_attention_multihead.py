...
class baseline(nn.Module):
    ...
    def forward(self, images, attention_maps, tasks):
        ...
        if self.training:
            predicts = self.training_process(images, attention_maps, tasks)
        else:
            predicts = self.inference(images, attention_maps, tasks)

        return predicts

    def training_process(self, images, attention_maps, tasks):
        # img = img.unsqueeze(0)
        ...
        for i in range(self.convLSTM_length):
            ...
            predict_head_rlts = self.object_head(features)
    ...
    def inference(self, images, attention_maps, tasks):
        # img = img.unsqueeze(0)
        batch, _, height, width = images.size()  # build a one-hot performance embedding
