...
parser.add_argument("--evaluation_dir", type=str, default="./assets/log_20201013_1859",
...
parser.add_argument("--min_length", type=int, default=2, help="Minimum length of the generated scanpath")
...
def main():
    ...
    test_dataset = AiR_evaluation(args.img_dir, args.fix_dir, None, args.bert_pretrained_dir,
    ...
    # human_metrics, human_metrics_std = human_evaluation(test_loader)
    ...
    with tqdm(total=len(test_loader) * repeat_num) as pbar_test:
        for i_batch, batch in enumerate(test_loader):
            tmp = [batch["images"], batch["fix_vectors"], batch["performances"], batch["bert_Qsemantics"]]
            tmp = [_ if not torch.is_tensor(_) else _.cuda() for _ in tmp]
            images, gt_fix_vectors, performances, bert_Qsemantics = tmp
    ...
    cur_metrics, cur_metrics_std = evaluation_performance_related(all_gt_fix_vectors, all_predict_fix_vectors,
