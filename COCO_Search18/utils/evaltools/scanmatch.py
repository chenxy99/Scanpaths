...
if __name__ == "__main__":
    ...
    # create a ScanMatch object.
    ScanMatchwithDuration = ScanMatch(Xres=1024, Yres=768, Xbin=12, Ybin=8, Offset=(0, 0), TempBin=100, Threshold=3.5)
    ScanMatchwithoutDuration = ScanMatch(Xres=1024, Yres=768, Xbin=12, Ybin=8, Offset=(0, 0), Threshold=3.5)
    ...
    # without
    sequence1_ = ScanMatchwithoutDuration.fixationToSequence(data1[:,:2]).astype(np.int32)
    sequence2_ = ScanMatchwithoutDuration.fixationToSequence(data2[:,:2]).astype(np.int32)
    sequence3_ = ScanMatchwithoutDuration.fixationToSequence(data3[:,:2]).astype(np.int32)
